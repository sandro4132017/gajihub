...
export default async function BasisDataGajiPage({
  ...
  if (!authUser || !canKelolaGajiInduk(authUser)) {
    return (
      <AksesDitolak pesan="Role kamu tidak berwenang mengelola basis data gaji." />
    );
  }
  ...
  const [jumlahIdentitas, totalAktif, tercakupAktif, terbaru, list] =
    await Promise.all([
      prisma.identitasWebGaji.count(),
      prisma.pegawai.count({ where: { statusPegawai: "AKTIF" } }),
      prisma.identitasWebGaji.count({
        where: { pegawai: { statusPegawai: "AKTIF" } },
      }),
  ...
  const tampil = hanyaBeda
    ? list.filter((r) => samakan(r.nama) !== samakan(r.pegawai.nama))
    : list;
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
      ...
      <h1 className="mt-2 text-xl font-extrabold tracking-tight text-ink">
        Basis Data Gaji (Web Gaji Kemenkeu)
      </h1>
      <p className="mt-1 max-w-3xl text-sm text-muted">
        Nama pegawai di berkas <strong>ADK</strong> diambil dari sini, bukan
        dari SIAP. Penulisan di Web Gaji berbeda (umumnya karena gelar), dan
        yang dikenali saat pembayaran adalah penulisan Web Gaji. Nama versi SIAP
        tetap dipakai di seluruh halaman lain - kolom itu tidak diubah.
      </p>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <div className="card p-4">
          ...
          <p className="mt-0.5 font-mono text-2xl font-bold text-ink">
            {jumlahIdentitas.toLocaleString("id-ID")}
          </p>
        </div>
        <div className="card p-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted">
            Pegawai aktif tercakup
          </p>
          <p className="mt-0.5 font-mono text-2xl font-bold text-ink">
        ...
        <div
          className={`card p-4 ${belumTercakup > 0 ? "border-gold/50 bg-gold-tint" : ""}`}
        >
          <p className="text-xs font-semibold uppercase tracking-wide text-muted">
            Memakai nama SIAP (cadangan)
          </p>
          <p className="mt-0.5 font-mono text-2xl font-bold text-ink">
            {belumTercakup.toLocaleString("id-ID")}
          </p>
        </div>
      </div>
      ...
      {terbaru && (
        <p className="mt-2 text-xs text-muted">
          Unggahan terakhir:{" "}
          <strong>{terbaru.sourceFileName ?? "(tanpa nama berkas)"}</strong>{" "}
          oleh {terbaru.diunggahOleh.nama},{" "}
          {terbaru.diunggahPada.toLocaleString("id-ID")}
        </p>
      )}
      ...
      <div className="card mt-6 p-4">
        <form method="get" className="flex flex-wrap items-end gap-3">
          ...
          <Link
            href={
              hanyaBeda
                ? `/ppabp/basis-data-gaji${cari ? `?q=${encodeURIComponent(cari)}` : ""}`
                : `/ppabp/basis-data-gaji?beda=1${cari ? `&q=${encodeURIComponent(cari)}` : ""}`
            }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-line bg-surface-2 text-xs font-bold uppercase tracking-wide text-muted">
              ...
              <th className="col-nama px-3 py-2.5">
                Nama di Web Gaji (dipakai ADK)
              </th>
            ...
            {tampil.map((r) => {
              ...
              return (
                <tr key={r.id} className="border-b border-line-2">
                  ...
                  <td
                    className={`col-nama px-3 py-2.5 ${beda ? "text-muted" : "text-ink-2"}`}
                  >
                    {r.pegawai.nama}
                  </td>
                  <td className="col-nama px-3 py-2.5 font-semibold text-ink">
                    {r.nama}
                  </td>
      ...
      {!hanyaBeda && jumlahIdentitas > tampil.length && (
        <p className="mt-2 text-xs text-muted">
          Menampilkan {tampil.length.toLocaleString("id-ID")} dari{" "}
          {jumlahIdentitas.toLocaleString("id-ID")} baris - pakai pencarian
          untuk mempersempit.
        </p>
      )}
    </main>
